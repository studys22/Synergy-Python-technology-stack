...
for key, val in pets.items():
    ...
    print(f'Это {val['Вид питомца']} по кличке "{key}". Возраст питомца: {val['Возраст питомца']} {"год" if ageP%10==1 else ("года" if ageP % 10 in {2, 3, 4} else "лет")}. Имя владельца: {val['Имя владельца']}')
