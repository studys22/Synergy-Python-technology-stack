...
for key, val in pets.items():
    ...
    print(
        f'Это {val["Вид питомца"]} по кличке "{key}". Возраст питомца: {val["Возраст питомца"]} {"лет" if (ageP % 100) in {11, 12, 13, 14} else ("год" if (ageP % 10 == 1) else ("года" if (ageP % 10) in {2, 3, 4} else "лет"))}. Имя владельца: {val["Имя владельца"]}'
    )
